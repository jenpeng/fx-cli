...
// 带颜色的日志输出
const logWithColor = (level, message, module = 'pushPluginService') => {
 ...
 */
const getProjectPath = async (pluginPath) => {
...
/**
 * 从服务器获取插件信息
 * @param {string} apiName - 插件的API名称
 * @returns {Promise<Object>} 插件信息
 */
const getPlugin = async (apiName) => {
  try {
    logger.info(`正在从服务器获取插件信息: ${apiName}`);
    
    // 使用fetchComponents方法获取插件列表
    const plugins = await api.fetchComponents('plugin', apiName);
    
    // 查找匹配的插件
    const plugin = plugins.find(plg => 
      plg.apiName === apiName || 
      plg.api_name === apiName ||
      plg.name === apiName
    );
    
    if (plugin) {
      logger.info(`成功获取插件信息: ${plugin.name} (${plugin.apiName || plugin.api_name})`);
      return plugin;
    } else {
      logger.warn(`未找到插件: ${apiName}`);
      return null;
    }
  } catch (error) {
...
/**
 * 推送插件代码
 * @param {string} pluginPath - 插件目录路径或单个文件路径
 * @param {string} type - 类型：plugin
 * @param {string} singleFilePath - 单个文件路径（可选，用于推送单个文件）
 * @returns {Promise<Object>} 推送结果
 */
const pushPlugin = async (pluginPath, type = 'plugin', singleFilePath = null) => {
  try {
    logger.info(`开始推送${type}: ${pluginPath}`);
    ...
    // 根据类型使用固定的XML文件名
    const xmlFileName = type === 'plugin' ? 'plugin.xml' : `${pluginName}.xml`;
    const xmlPath = path.join(pluginPath, xmlFileName);
    ...
    // 设置插件ID为插件名称，确保符合coding变量命名规则并带有__c后缀
    let sanitizedPluginName = pluginName;
    
    // 移除__c后缀（如果存在）以便统一处理
    if (sanitizedPluginName.endsWith('__c')) {
      sanitizedPluginName = sanitizedPluginName.slice(0, -3);
    }
    
    // 清理插件名称，确保符合coding变量命名规则：
    ...
    // 确保插件ID带有__c后缀
    let pluginId = `${sanitizedPluginName}__c`;
    
    // 与extension完全一致：检查源代码目录
    const sourceFilesPath = path.join(pluginPath, 'sourceFiles');
    const fileTreePath = path.join(pluginPath, 'fileTree');
    ...
    const staticDirPath = path.join(pluginPath, 'static');
    ...
    const projectPath = await getProjectPath(pluginPath);
    ...
    // 智能判断：检查unchangeableJson.json文件中是否已存在该插件
    if (fs.existsSync(unchangeableJsonPath)) {
      ...
      if (unchangeableJson[originalPluginKey]) {
        pluginId = unchangeableJson[originalPluginKey].apiName;
        updateTime = unchangeableJson[originalPluginKey].updateTime;
        logger.info(`插件 ${pluginName} 已存在于unchangeableJson.json中，apiName: ${pluginId}, updateTime: ${updateTime}`);
      } else if (unchangeableJson[sanitizedPluginKey]) {
        pluginId = unchangeableJson[sanitizedPluginKey].apiName;
        updateTime = unchangeableJson[sanitizedPluginKey].updateTime;
        logger.info(`插件 ${sanitizedPluginName} 已存在于unchangeableJson.json中，apiName: ${pluginId}, updateTime: ${updateTime}`);
      } else {
        logger.info(`插件 ${pluginName} 不存在于unchangeableJson.json中，将创建新插件`);
      }
    } else {
      logger.info(`unchangeableJson.json文件不存在，将创建新插件`);
    }
    ...
    const data = {
      ...
      apiName: pluginId,  // 与extension保持一致：使用apiName而不是id
      ...
      type: 'plugin',  // 与extension完全一致：type固定为'plugin'
      updateTime: updateTime  // 与extension完全一致：从unchangeableJson获取updateTime
    };
    
    // 对于新插件，设置updateTime为0，让服务端自动生成
    // 对于已存在插件，使用从unchangeableJson获取的updateTime
    if (!fs.existsSync(unchangeableJsonPath)) {
      // unchangeableJson.json文件不存在，说明是新项目，所有插件都是新插件
      data.updateTime = 0;
      ...
      // 检查插件是否已存在于unchangeableJson.json中
      if (unchangeableJson[originalPluginKey]) {
        // 插件已存在，使用从unchangeableJson获取的updateTime
        data.updateTime = unchangeableJson[originalPluginKey].updateTime;
        logger.info(`插件 ${pluginName} 已存在于unchangeableJson.json中，使用updateTime: ${data.updateTime}`);
      } else if (unchangeableJson[sanitizedPluginKey]) {
        // 插件已存在，使用从unchangeableJson获取的updateTime
        data.updateTime = unchangeableJson[sanitizedPluginKey].updateTime;
        logger.info(`插件 ${sanitizedPluginName} 已存在于unchangeableJson.json中，使用updateTime: ${data.updateTime}`);
      } else {
        // 插件不存在，设置updateTime为0，让服务端自动生成
        data.updateTime = 0;
        logger.info(`插件 ${pluginName} 不存在于unchangeableJson.json中，设置updateTime为0，让服务端自动生成`);
      }
    }
    ...
    try {
      ...
      errorResult = errorHandler.handleApiError(response, '推送插件');
      ...
      if (errorResult.isError) {
        ...
        if (errorResult.message && errorResult.message.includes('已存在相同的apiName')) {
          ...
          const retryErrorResult = errorHandler.handleApiError(retryResponse, '推送插件');
      ...
      if (error.message && error.message.includes('系统出现异常') && data.updateTime !== 0) {
        ...
        // 如果是"当前代码在线上有更高版本"错误，尝试从服务器获取最新插件信息并重试
        logger.warn(`检测到版本冲突，尝试从服务器获取最新插件信息...`);
        
        try {
          // 从服务器获取最新插件信息
          const pluginInfo = await getPlugin(pluginId);
          
          if (pluginInfo && pluginInfo.updateTime) {
            ...
            try {
              ...
              if (fs.existsSync(unchangeableJsonPath)) {
                ...
                // 智能判断：根据插件是否已存在于unchangeableJson.json中决定使用哪个键名
                let pluginKey;
                if (unchangeableJson[originalPluginKey]) {
                  pluginKey = originalPluginKey;
                } else if (unchangeableJson[sanitizedPluginKey]) {
                  pluginKey = sanitizedPluginKey;
                } else {
                  pluginKey = originalPluginKey;
                }
                ...
                if (unchangeableJson[pluginKey]) {
                  ...
                  logger.info(`更新本地unchangeableJson.json中的插件 ${pluginKey} 的updateTime为 ${latestUpdateTime}`);
            ...
            // 无法从服务器获取插件信息，使用本地updateTime重试
            logger.warn(`无法从服务器获取插件信息，使用本地updateTime=${updateTime}重试...`);
            data.updateTime = updateTime;
    ...
    try {
      ...
      if (fs.existsSync(unchangeableJsonPath)) {
        ...
        if (unchangeableJson[originalPluginKey]) {
          // 插件已存在，使用原始插件名称作为键名
          pluginKey = originalPluginKey;
          logger.info(`插件已存在，使用原始插件名称作为插件键名: ${pluginKey}`);
        } else if (unchangeableJson[sanitizedPluginKey]) {
          // 插件已存在，使用清理后的插件名称作为键名
          pluginKey = sanitizedPluginKey;
          logger.info(`插件已存在，使用清理后的插件名称作为插件键名: ${pluginKey}`);
        } else {
          // 插件不存在，使用原始插件名称作为键名
          pluginKey = originalPluginKey;
          logger.info(`插件不存在，使用原始插件名称作为插件键名: ${pluginKey}`);
        }
        ...
        if (unchangeableJson[pluginKey]) {
          // 更新现有的插件记录
          unchangeableJson[pluginKey].updateTime = latestUpdateTime;
          logger.info(`更新unchangeableJson.json中的插件 ${pluginKey} 的updateTime为 ${latestUpdateTime}`);
        } else {
          // 创建新的插件记录
          ...
          logger.info(`在unchangeableJson.json中创建新的插件记录 ${pluginKey}`);
        }
    ...
    return {
      success: true,
      name: pluginName,
      id: response.Value?.id || pluginId,  // 与extension保持一致：使用response.Value
      message: response.Value?.message || '推送成功'
    };
  } catch (error) {
    logger.error(`推送${type}失败 [${path.basename(pluginPath)}]: ${error.message}`);
    throw new Error(`推送${type}失败 [${path.basename(pluginPath)}]: ${error.message}`);
  }
};
...
const deployPlugin = async (pluginPath, type = 'plugin', env = 'dev') => {
  try {
    ...
    // 2. 然后构建插件 - 使用api.js中的buildComponent函数
